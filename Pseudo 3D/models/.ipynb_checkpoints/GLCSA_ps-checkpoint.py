...
class SliceInteraction(nn.Module):
    def __init__(self, in_channels, n_heads=4):
        super().__init__()
        self.n_heads = n_heads
        self.scale = (in_channels // n_heads) ** -0.5

        self.q_map = nn.Linear(in_channels, in_channels)
        self.k_map = nn.Linear(in_channels, in_channels)
        self.v_map = nn.Linear(in_channels, in_channels)
        self.sdp = DotProduct()

    def forward(self, x):
        b, hw, c = x.shape
        q = self.q_map(x).view(b, hw, self.n_heads, c // self.n_heads).permute(0, 2, 1, 3)
        k = self.k_map(x).view(b, hw, self.n_heads, c // self.n_heads).permute(0, 2, 1, 3)
        v = self.v_map(x).view(b, hw, self.n_heads, c // self.n_heads).permute(0, 2, 1, 3)

        attn = self.sdp(q, k, v, self.scale).permute(0, 2, 1, 3).reshape(b, hw, c)
        return attn
...
class GlobalLocalChannelAttention(nn.Module):
    def __init__(self, in_features, out_features, n_heads=4, window_size=1):
        ...
        # Global attention
        self.q_map_global = MainConv(in_channels=out_features, out_channels=out_features, groups=out_features)
        self.k_map_global = MainConv(in_channels=in_features, out_channels=in_features, groups=in_features)
        self.v_map_global = MainConv(in_channels=in_features, out_channels=in_features, groups=in_features)

        # Local attention
        if self.window_size > 0:
            self.q_map_local = MainConv(in_channels=out_features, out_channels=out_features, groups=out_features)
            self.k_map_local = MainConv(in_channels=in_features, out_channels=in_features, groups=in_features)
            self.v_map_local = MainConv(in_channels=in_features, out_channels=in_features, groups=in_features)

        # Cross-slice attention
        self.slice_attention = SliceInteraction(in_channels=out_features, n_heads=n_heads)

        self.projection = MainConv(in_channels=out_features, out_channels=out_features, groups=out_features)
        self.sdp = DotProduct()
    ...
    def forward(self, x):
        ...
        # Cross-slice attention
        slice_att = self.slice_attention(att)

        # Combine with cross-slice attention
        att = att + slice_att
...
class GlobalLocalSpatialAttentionAlternate(nn.Module):
    def __init__(self, in_features, out_features, n_heads=4, global_ratio=0.5, window_size=4):
        super().__init__()
        self.n_heads = n_heads
    ...
    def forward(self, x):
        ...
        # Local Attention using windowing
        local_q = self.local_q_map(q)
        local_k = self.local_k_map(k)
        local_v = self.local_v_map(v)  
        
        # Reshape to apply windowing
        b, hw, c = local_q.shape
        c_v = local_v.shape[2]
        window_size = self.window_size
        num_windows = hw // window_size

        local_q_windows = local_q.reshape(b, num_windows, window_size, c)
        local_k_windows = local_k.reshape(b, num_windows, window_size, c)
        local_v_windows = local_v.reshape(b, num_windows, window_size, c_v)
        
        scale_local = (c // self.n_heads) ** -0.5
        
        local_q_windows = local_q_windows.reshape(b, num_windows, window_size, self.n_heads, c // self.n_heads).permute(0, 3, 1, 2, 4).reshape(b * self.n_heads, num_windows, window_size, c // self.n_heads)
        local_k_windows = local_k_windows.reshape(b, num_windows, window_size, self.n_heads, c // self.n_heads).permute(0, 3, 1, 2, 4).reshape(b * self.n_heads, num_windows, window_size, c // self.n_heads)
        local_v_windows = local_v_windows.reshape(b, num_windows, window_size, self.n_heads, c_v // self.n_heads).permute(0, 3, 1, 2, 4).reshape(b * self.n_heads, num_windows, window_size, c_v // self.n_heads)

        local_att_windows = self.sdp(local_q_windows, local_k_windows, local_v_windows, scale_local)
        
        local_att_windows = local_att_windows.reshape(b, self.n_heads, num_windows, window_size, c_v // self.n_heads).permute(0, 2, 3, 1, 4).reshape(b, hw, c_v)
        
        # Combine Global and Local Attention
        combined_att = (1 - self.global_ratio) * local_att_windows + self.global_ratio * global_att

        # Cross-slice attention
        slice_att = self.slice_attention(combined_att)

        # Combine with cross-slice attention
        combined_att = combined_att + slice_att
...
class GLCSA(nn.Module):
    def __init__(self, features, strides, emb='patch', patch=8, channel_att=True, spatial_att=True, n_blocks=1, channel_head=[1, 1, 1, 1], spatial_head=[4, 4, 4, 4], fusion='sequential'):   #default embedding set to pool
        super().__init__()
